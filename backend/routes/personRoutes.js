...
router.get("/", async (req, res) => {
  try {
    ...
    const peopleWithUrls = people.map(person => ({
      ...person,
      photos: person.photos ? person.photos.map(photo => ({
        fileName: photo,
        url: getSupabaseImageUrl(photo)
      })) : [],
      profilePictureUrl: person.profilePicture ? getSupabaseImageUrl(person.profilePicture) : null
    }));
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (person.photos && person.photos.length > 0) {
      console.log(`🗑️ Deleting ${person.photos.length} photos from Supabase...`);
